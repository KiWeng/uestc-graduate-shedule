...
(function () {
    ...
    let generate_button = () => { // 按钮生成
        ...
        if (xkjg) {
            ...
            inserted_node.style = 'width:200px;margin-left:15px'
            document.getElementById("xq").insertAdjacentElement('afterend', inserted_node)
        ...
        if (xkjg) {
            document.getElementById('datepicker').insertAdjacentElement('afterend', download_ics)
            download_ics.addEventListener("click", parse_xkjg);
        } else if (kskcxx) {
            document.getElementById('xqbh').insertAdjacentElement('afterend', download_ics)
            download_ics.addEventListener("click", parse_kskcxx);
        }
    }

    let insert_weekly_events = (name, description, location, start_time, end_time, duration) => {
    ...
    let parse_kskcxx = () => {
        const exams = document.getElementById('sample-table-1').getElementsByTagName("tbody")[0].rows;
        for (let exam of exams) {
            ...
            const regex = /(\d+)-(\d+)-(\d+)日 (\d+):(\d+)~(\d+):(\d+)/ug
            const time_info = regex.exec(exam_info['time'])

            // 起止时间
            const event_start_time = new Date(parseInt(time_info[1]), parseInt(time_info[2]) - 1, parseInt(time_info[3]), parseInt(time_info[4]), parseInt(time_info[5]));
            const event_end_time = new Date(parseInt(time_info[1]), parseInt(time_info[2]) - 1, parseInt(time_info[3]), parseInt(time_info[6]), parseInt(time_info[7]));

            cal.addEvent(exam_info['name'], description, exam_info['room'], event_start_time.toUTCString(), event_end_time.toUTCString());
        }
    ...
    let parse_xkjg = () => {
        const courses = document.getElementById('sample-table-1').getElementsByTagName("tbody")[0].rows;
        for (let course of courses) {
            ...
            for (let line of course_info['time_location'].split('\n')) {
                ...
                for (const element of array) {
                    ...
                    event_end_time.setDate(event_end_time.getDate() + (start_week - 1) * 7 + weekday);
